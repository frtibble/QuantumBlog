...
namespace Microsoft.Quantum.Examples.Teleportation {
    ...
    /// # Summary
    /// Uses teleportation to send a quantum state from one qubit
    /// to another. The state we will send will be the state
	/// 1/sqrt(2)(|0〉+|1〉), which upon measurement results in
	/// a 50-50 split of '0' and '1' (see the Measurement sample
	/// for further info).
    ///
    /// ## Output
    /// The result of a Z-basis measurement on the teleported qubit,
    /// which should return a 50-50 split of '0' and '1' if the state
	/// has been successfully teleported.
	operation TeleportQuantumMessage () : Result {
        body {
            mutable result = Zero;
